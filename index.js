...
var query = "abarrotes,oxxo,seven,k circle en ";
var posUsuario;
var alcaldiaActual;
var ultimaAlcaldia;
var porAlcaldia = {};
...
function showResultados() {
    ...
    ultimoDesplegar=aDesplegar[aDesplegar.length-1];
...
function getBoozeData(map, loc, callback) {
    ...
    for (alcaldia of listaDeAlcaldias) {
        if (alcaldias[alcaldia] == false) {//no tiene ley seca
            alcaldiaActual = alcaldia;
            getBoozeAlcaldiaData(map, alcaldia);
        }
    }
}
function getBoozeAlcaldiaData(map, alcaldia) {
    request = {
...
function textSearchCallback(results, status) {
    if (status == google.maps.places.PlacesServiceStatus.OK) {
        ...
        if(alcaldiaActual==ultimaAlcaldia){
            showResultados();
        }
    }
}
